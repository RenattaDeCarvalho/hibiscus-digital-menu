...
export function AdminDashboard() {
  ...
  const handleToggleActive = useCallback(async (item: any, active: boolean) => {
    try {
  ...
  const handleMoveCategory = useCallback(
    async (catId: string, direction: "up" | "down") => {
      try {
    ...
    <div className="min-h-screen bg-[hsl(0,0%,5%)]">
      ...
      <header className="sticky top-0 z-50 bg-[hsl(0,0%,5%)]/95 backdrop-blur-sm border-b border-[hsl(0,0%,12%)]">
        <div className="max-w-[1000px] mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-serif text-[hsl(40,60%,55%)] text-lg">
                Administração
              </h1>
              <p className="text-[hsl(40,10%,40%)] text-[10px]">
                {session?.user?.email ?? ""}
              </p>
            </div>
          </div>
      ...
      <div className="max-w-[1000px] mx-auto px-4 py-6">
        ...
        <div className="mb-6 p-4 rounded-xl bg-[hsl(0,0%,8%)] border border-[hsl(0,0%,12%)]">
          <h3 className="font-serif text-[hsl(40,20%,85%)] text-base mb-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-1">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-serif text-[hsl(40,20%,85%)] text-lg">
                Categorias
              </h2>
              <button
                onClick={() =>
                ...
                >
                  <div className="flex flex-col gap-0.5">
                    <button
                      ...
                      className="text-[hsl(40,10%,40%)] hover:text-[hsl(40,60%,55%)]"
                    >
                  ...
                  >
                    <span className="text-sm text-[hsl(40,20%,85%)]">
                      {cat?.namePt ?? ""}
                    </span>
                  </button>
                  <button
                  ...
                  <button
                    ...
                    className="text-[hsl(40,10%,40%)] hover:text-red-400"
                  >
            ...
            {selectedCat ? (
              <>
                ...
                <div className="space-y-2">
                  {(items ?? [])?.map?.((item: any, idx: number) => (
                    ...
                    >
                      <input
                        ...
                        className="w-5 h-5 accent-[hsl(40,60%,55%)] cursor-pointer shrink-0"
                      />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-sm text-[hsl(40,60%,55%)] font-serif">
                            R${" "}
                            {item?.price?.toFixed?.(2)?.replace?.(".", ",") ??
                              "0,00"}
                          </span>
                        </div>
                        <p className="text-[hsl(40,10%,40%)] text-xs truncate mt-0.5">
                          {item?.descPt ?? ""}
                        </p>
                      </div>
                      <button
                  ...
                  {(items?.length ?? 0) === 0 ? (
                    <p className="text-center text-[hsl(40,10%,40%)] text-sm py-8">
...
function CategoryModal({
  ...
  return (
    ...
    >
      <div
        className="bg-[hsl(0,0%,8%)] rounded-xl w-full max-w-lg max-h-[85vh] overflow-y-auto p-6"
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={onClose}
            className="text-[hsl(40,10%,40%)] hover:text-[hsl(40,20%,70%)]"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="space-y-3">
          <InputField
            label="Nome (PT)"
            value={form?.namePt ?? ""}
            onChange={(v: string) => handleChange("namePt", v)}
          />
          <InputField
            label="Nome (EN)"
            value={form?.nameEn ?? ""}
            onChange={(v: string) => handleChange("nameEn", v)}
          />
          <InputField
            label="Nome (ES)"
            value={form?.nameEs ?? ""}
            onChange={(v: string) => handleChange("nameEs", v)}
          />
          <InputField
            label="Slug"
            value={form?.slug ?? ""}
            onChange={(v: string) => handleChange("slug", v)}
          />
          <InputField
            label="Descrição (PT)"
            value={form?.descPt ?? ""}
            onChange={(v: string) => handleChange("descPt", v)}
          />
          <InputField
            label="Descrição (EN)"
            value={form?.descEn ?? ""}
            onChange={(v: string) => handleChange("descEn", v)}
          />
          <InputField
            label="Descrição (ES)"
            value={form?.descEs ?? ""}
            onChange={(v: string) => handleChange("descEs", v)}
          />

          {/* Icon Picker */}
          <div>
            <label className="block text-[hsl(40,10%,55%)] text-xs mb-2">
              Ícone da Categoria
            </label>
                ...
                >
                  ...
                  {form.iconUrl === icon.url && (
                    <div className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-[hsl(40,60%,55%)] flex items-center justify-center">
                      <svg
                        className="w-2.5 h-2.5 text-[hsl(0,0%,5%)]"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        strokeWidth={3}
            ...
            {form.iconUrl && (
              <p className="text-[hsl(40,10%,40%)] text-[10px] mt-1.5 truncate">
                Selecionado:{" "}
                {AVAILABLE_ICONS.find((i) => i.url === form.iconUrl)?.label ??
                  form.iconUrl}
              </p>
            )}
          </div>
        ...
        <div className="flex gap-3 mt-6">
          <button
            onClick={onClose}
            className="flex-1 py-2.5 rounded-lg border border-[hsl(0,0%,18%)] text-[hsl(40,10%,60%)] text-sm hover:bg-[hsl(0,0%,12%)] transition-colors"
          >
            Cancelar
          </button>
          <button
    ...
    >
      ...
      >
        ...
        <div className="space-y-3">
          ...
          />
          <div>
            <label className="text-[hsl(40,10%,50%)] text-xs mb-1 block">
              Preço (R$)
            </label>
            <input
              type="number"
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                handleChange("price", parseFloat(e?.target?.value ?? "0") || 0)
              }
